...
prisma.order.findMany({
where: { organizationId: organization.id, status: { not: "cancelled" }, ...dateWhere },
select: { totalAmount: true, amountPaid: true, taxAmount: true, createdAt: true },
}),
prisma.order.groupBy({
by: ["customerId"],
where: { organizationId: organization.id, status: { not: "cancelled" }, ...dateWhere },
...
by: ["method"],
where: { organizationId: organization.id, type: "payment", ...(hasDateFilter ? { createdAt: dateFilter } : {}) },
_sum: { amount: true },
_count: { _all: true },
}),
prisma.payment.aggregate({
where: { organizationId: organization.id, ...(hasDateFilter ? { createdAt: dateFilter } : {}) },
_sum: { tip: true },
}),
prisma.payment.aggregate({
where: { organizationId: organization.id, type: "refund", ...(hasDateFilter ? { createdAt: dateFilter } : {}) },
_sum: { amount: true },
}),
...
const monthly = new Map<string, { revenue: number; collected: number; orders: number; tax: number }>();
for (const o of salesOrders) {
const d = new Date(o.createdAt);
const key = `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}`;
const entry = monthly.get(key) || { revenue: 0, collected: 0, orders: 0, tax: 0 };
entry.revenue += o.totalAmount || 0;
entry.collected += o.amountPaid || 0;
entry.tax += o.taxAmount || 0;
entry.orders += 1;
...
const totalTipsCollected = tipAgg._sum.tip || 0;
const totalRefunds = refundAgg._sum.amount || 0;
const totalTaxCollected = taxAgg._sum.taxAmount || 0;
const taxOrders = openOrders.filter((o) => (o.taxAmount || 0) > 0.009).sort((a, b) => (b.taxAmount || 0) - (a.taxAmount || 0));
...
<div className="text-2xl font-bold text-gray-900">${totalTipsCollected.toFixed(2)}</div>
</div>
<div className="bg-white shadow rounded-lg p-4 border-l-4 border-red-500">
<div className="text-sm text-gray-500">Refunds Issued</div>
<div className="text-2xl font-bold text-gray-900">${totalRefunds.toFixed(2)}</div>
</div>
</div>
...
<th className="px-6 py-2 font-medium">Transactions</th>
<th className="px-6 py-2 font-medium">Total Collected</th>
</tr>
</thead>
<tbody className="divide-y divide-gray-100">
{paymentMethodAgg.map((m) => (
<tr key={m.method}>
<td className="px-6 py-2">{methodLabels[m.method] || m.method}</td>
<td className="px-6 py-2">{m._count._all}</td>
<td className="px-6 py-2">${(m._sum.amount || 0).toFixed(2)}</td>
</tr>
))}
{paymentMethodAgg.length === 0 && (
<tr>
<td colSpan={3} className="px-6 py-4 text-gray-500">No payments recorded yet.</td>
</tr>
)}
</tbody>
</table>
...
<tr key={p.id}>
<td className="px-6 py-2">{new Date(p.createdAt).toLocaleDateString()}</td>
<td className="px-6 py-2">{p.order.orderNumber}</td>
<td className="px-6 py-2">{p.order.customer.firstName} {p.order.customer.lastName}</td>
<td className="px-6 py-2 capitalize">{p.type}</td>
<td className="px-6 py-2">{methodLabels[p.method] || p.method}</td>
<td className={"px-6 py-2 " + (p.type === "refund" ? "text-red-600" : "text-gray-900")}>{p.type === "refund" ? "-" : ""}${p.amount.toFixed(2)}</td>
</tr>
))}
{recentPayments.length === 0 && (
<tr>
<td colSpan={6} className="px-6 py-4 text-gray-500">No payments recorded yet.</td>
</tr>
)}
</tbody>
</table>
...
<div className="text-2xl font-bold text-gray-900">${totalTaxCollected.toFixed(2)}</div>
</div>
<div className="bg-white shadow rounded-lg p-4 border-l-4 border-blue-600">
...
<div className="bg-white shadow rounded-lg overflow-hidden mb-8">
<div className="px-6 py-4 border-b border-gray-200">
<h2 className="text-lg font-semibold text-gray-900">Tax Collected by Month</h2>
</div>
<table className="w-full text-sm">
<thead>
<tr className="text-left text-gray-500 border-b border-gray-200">
<th className="px-6 py-2 font-medium">Month</th>
<th className="px-6 py-2 font-medium">Orders</th>
<th className="px-6 py-2 font-medium">Tax Collected</th>
</tr>
</thead>
<tbody className="divide-y divide-gray-100">
{monthlyRows.map((m) => (
...
<td className="px-6 py-2">{m.label}</td>
<td className="px-6 py-2">{m.orders}</td>
<td className="px-6 py-2">${m.tax.toFixed(2)}</td>
</tr>
))}
{monthlyRows.length === 0 && (
<tr>
<td colSpan={3} className="px-6 py-4 text-gray-500">No sales in this range yet.</td>
</tr>
)}
</tbody>
</table>
...
<div className="bg-white shadow rounded-lg overflow-hidden mb-8">
<div className="px-6 py-4 border-b border-gray-200">
<h2 className="text-lg font-semibold text-gray-900">Orders with Tax Charged</h2>
</div>
<table className="w-full text-sm">
<thead>
<tr className="text-left text-gray-500 border-b border-gray-200">
<th className="px-6 py-2 font-medium">Order</th>
<th className="px-6 py-2 font-medium">Customer</th>
<th className="px-6 py-2 font-medium">Subtotal</th>
<th className="px-6 py-2 font-medium">Tax</th>
<th className="px-6 py-2 font-medium">Total</th>
</tr>
</thead>
<tbody className="divide-y divide-gray-100">
{taxOrders.map((o) => (
<tr key={o.id}>
<td className="px-6 py-2">{o.orderNumber}</td>
<td className="px-6 py-2">{o.customer.firstName} {o.customer.lastName}</td>
<td className="px-6 py-2">${o.subtotal.toFixed(2)}</td>
<td className="px-6 py-2">${o.taxAmount.toFixed(2)}</td>
<td className="px-6 py-2">${o.totalAmount.toFixed(2)}</td>
</tr>
))}
{taxOrders.length === 0 && (
<tr>
<td colSpan={5} className="px-6 py-4 text-gray-500">No orders with tax charged in this range.</td>
</tr>
)}
</tbody>
</table>
