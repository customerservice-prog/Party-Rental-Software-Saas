...
export default function ReadinessPanel(){return <section className="rounded-2xl border border-slate-200 bg-white shadow-sm"><div className="border-b border-slate-100 px-5 py-4"><h2 className="font-black">Core rental system</h2><p className="text-xs text-slate-500">Fast access to the systems that run a real rental operation.</p></div><div className="grid gap-px bg-slate-100 sm:grid-cols-2">{checks.map(c=><Link key={c.href} href={c.href} className="bg-white p-4 hover:bg-blue-50/50"><div className="flex items-center justify-between gap-3"><b className="text-sm text-slate-900">{c.label}</b><span className="text-blue-600">→</span></div><p className="mt-1 text-[11px] leading-4 text-slate-500">{c.desc}</p></Link>)}</div></section>}
