...
export default async function DashboardHomePage({searchParams: searchParamsPromise}:{searchParams:Promise<{year?:string;month?:string}>}){
 ...
 return <div className="space-y-4 pb-4">
  <PageHeading eyebrow={dates.today.toLocaleDateString("en-US",{weekday:"long",month:"long",day:"numeric",year:"numeric",timeZone:"UTC"})} title="Dashboard" description="Bookings, payments, inventory, and today’s rental operations at a glance." actions={<Link href="/dashboard/scheduling" className="tenant-button"><Icon name="calendar" className="h-4 w-4"/>Open calendar</Link>}/>
  ...
  <section aria-label="Business summary" className="grid grid-cols-2 gap-3 lg:grid-cols-4">
   <MetricCard label="Upcoming Events" value={weekCount} detail="Confirmed & active · next 7 days" icon="calendar" href="/dashboard/scheduling"/>
   <MetricCard label="Payments" value={money(netPayments)} detail="Net recorded payments · last 30 days" icon="wallet" href="/dashboard/reports"/>
   <MetricCard label="Balance Due" value={money(balance)} detail="Outstanding customer balances" icon="orders" href="/dashboard/orders?balance=unpaid"/>
   <MetricCard label="Inventory Items" value={itemCount} detail="Active rental catalog items" icon="box" href="/dashboard/inventory"/>
  </section>
