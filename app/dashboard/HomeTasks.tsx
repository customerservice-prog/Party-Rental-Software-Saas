...
export default function HomeTasks() {
  ...
  return <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
    <div className="border-b border-slate-100 px-5 py-4">
      <div className="flex items-start justify-between gap-4"><div><div className="flex items-center gap-2"><h3 className="font-black text-slate-950">Action center</h3>{openTasks.length > 0 && <span className="rounded-full bg-slate-950 px-2 py-0.5 text-[10px] font-black text-white">{openTasks.length}</span>}</div><p className="mt-1 text-xs text-slate-500">What needs attention next</p></div>{overdue > 0 && <span className="animate-pulse rounded-full border border-red-100 bg-red-50 px-2.5 py-1 text-[10px] font-black uppercase tracking-wide text-red-700">{overdue} overdue</span>}</div>
    </div>
    <div className="p-5">
      <div className="flex gap-2"><input value={draft} onChange={(e)=>setDraft(e.target.value)} onKeyDown={(e)=>{ if(e.key==="Enter") addTask(); }} placeholder="Add something your team needs to do…" className="min-w-0 flex-1 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm outline-none transition focus:border-blue-400 focus:bg-white focus:ring-4 focus:ring-blue-50"/><button onClick={addTask} disabled={saving || !draft.trim()} className="rounded-xl bg-slate-950 px-3.5 py-2.5 text-sm font-black text-white transition hover:bg-blue-600 disabled:cursor-not-allowed disabled:opacity-40">{saving ? "…" : "+"}</button></div>
