...
export default function HomeTasks() {
  ...
  return <section className="friendly-admin-card !p-0 overflow-hidden">
    <div className="border-b border-slate-100 px-5 py-4">
      <div className="flex items-start justify-between gap-4"><div><div className="flex items-center gap-2"><h3 className="font-bold text-slate-950">Team tasks</h3>{openTasks.length > 0 && <span className="rounded-full bg-slate-950 px-2 py-0.5 text-[10px] font-bold text-white">{openTasks.length}</span>}</div><p className="mt-1 text-xs text-slate-500">What needs attention next</p></div>{overdue > 0 && <span className="rounded-full border border-red-100 bg-red-50 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wide text-red-700">{overdue} overdue</span>}</div>
    </div>
    <div className="p-5">
