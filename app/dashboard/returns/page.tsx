...
export default async function ReturnsPage(){
  ...
  return <div className="friendly-admin-page is-wide">
    <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
      <div><h1 className="text-2xl font-bold text-dark">Returns & Damage</h1><p className="mt-1 text-sm text-slate-500">Reconcile every order, quarantine damaged or missing quantities, and restore repaired equipment to availability.</p></div>
      <div className="flex flex-wrap gap-2"><Link href="/dashboard/warehouse" className="friendly-admin-secondary">Warehouse scan</Link><Link href="/dashboard/operations" className="friendly-admin-secondary">← Operations</Link></div>
    </div>
    <div className="grid grid-cols-2 gap-3 lg:grid-cols-5">
      {[["Recent orders",orders.length],["Items to inspect",recentInventory.length],["Open quantity holds",qtyHolds.quantity],["Missing catalog items",items.filter(i=>i.status==="missing").length],["Repair / damaged",items.filter(i=>i.status==="damaged"||i.status==="needs_repair").length]].map(([a,b])=><div key={String(a)} className="friendly-admin-kpi"><div className="text-[10px] font-black uppercase text-slate-400">{a}</div><div className="mt-1 text-2xl font-black">{b}</div></div>)}
    </div>
    ...
    <div className="grid gap-5 xl:grid-cols-[1.15fr_.85fr]">
      ...
      <aside className="space-y-4">
        <section className="friendly-admin-card !border-amber-200 !bg-amber-50">
        ...
        <section className="friendly-admin-card !bg-slate-50">
    ...
    <section className="friendly-admin-card !p-0">
