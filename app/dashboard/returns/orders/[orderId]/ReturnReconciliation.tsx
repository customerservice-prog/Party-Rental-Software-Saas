...
export default function ReturnReconciliation({orderId,orderNumber}:{orderId:string;orderNumber:string}){
  ...
  return <div className="space-y-5">
    <section className="grid grid-cols-2 gap-3 md:grid-cols-4">
      {[["Expected",totals.expected,"text-slate-700"],["Returned OK",totals.returned,"text-emerald-700"],["Damaged",totals.damaged,"text-amber-700"],["Missing",totals.missing,"text-rose-700"]].map(([label,value,cls])=><div key={String(label)} className="friendly-admin-kpi"><div className="text-[9px] font-bold uppercase text-slate-400">{label}</div><div className={`mt-1 text-3xl font-bold ${cls}`}>{value}</div></div>)}
    </section>
    <section className="friendly-admin-card !mb-0">
      <div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="font-bold">Return progress</h2><p className="text-xs text-slate-500">Order #{orderNumber} · serialized scans and manual quantity counts stay synchronized.</p></div><b className={reconciled>=totals.expected&&totals.expected>0?"text-emerald-700":"text-slate-600"}>{reconciled}/{totals.expected}</b></div>
    ...
    <section className="friendly-admin-card !mb-0 !p-0 overflow-hidden">
      <div className="border-b border-slate-100 px-5 py-4"><h2 className="font-bold">Physical equipment</h2><p className="text-xs text-slate-500">Packages are already expanded into actual warehouse items. Damaged or missing quantity-only units are held out of future availability until resolved.</p></div>
