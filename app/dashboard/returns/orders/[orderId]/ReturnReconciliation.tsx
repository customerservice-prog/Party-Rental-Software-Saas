    ...
    <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
      ...
      <div className="divide-y divide-slate-100">{state.resources.map(row=>{
        ...
        return <div key={row.itemId} className="p-5">
          ...
          <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-4">{[["Loaded / Out","loadedQty",minimum.loaded],["Returned OK","returnedQty",minimum.returned],["Damaged","damagedQty",minimum.damaged],["Missing","missingQty",minimum.missing]].map(([label,key,min])=><label key={String(key)} className="text-[10px] font-black uppercase text-slate-400">{label}<input type="number" min={Number(min)} max={row.expectedQty} value={Number(draft[key as keyof Draft])||0} onChange={e=>setField(row.itemId,key as keyof Draft,Math.max(Number(min),Math.floor(Number(e.target.value)||0)))} className="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm font-black text-slate-900"/></label>)}</div>
          <textarea value={draft.notes} onChange={e=>setField(row.itemId,"notes",e.target.value)} placeholder="Damage details, missing pieces, repair notes…" className="mt-3 min-h-[62px] w-full rounded-xl border border-slate-200 p-3 text-xs"/>
          <div className="mt-3 flex items-center justify-between gap-3"><span className="text-[10px] text-slate-400">Returned + damaged + missing cannot exceed {row.expectedQty}.</span><button disabled={saving===row.itemId} onClick={()=>save(row)} className="rounded-xl bg-slate-950 px-4 py-2.5 text-xs font-black text-white disabled:opacity-50">{saving===row.itemId?"Saving…":"Save counts"}</button></div>
        </div>;
      })}</div>
    </section>
  </div>;
