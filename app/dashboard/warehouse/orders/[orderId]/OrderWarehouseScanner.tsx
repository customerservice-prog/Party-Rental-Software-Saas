    ...
    <section className="grid gap-5 xl:grid-cols-[.7fr_1.3fr]">
      <div className="space-y-4">
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          <div className="text-[10px] font-black uppercase tracking-[.16em] text-blue-600">Order #{orderNumber}</div>
          <h2 className="mt-1 text-xl font-black">Scan serialized equipment</h2>
          ...
          <div className="mt-4 grid grid-cols-2 gap-2">
            {ACTIONS.map(a=><button key={a.value} onClick={()=>{setAction(a.value);setTimeout(()=>inputRef.current?.focus(),20)}} className={`rounded-xl border p-3 text-left transition ${action===a.value?"border-blue-600 bg-blue-600 text-white":"border-slate-200 bg-white text-slate-700 hover:bg-slate-50"}`}><b className="block text-xs">{a.label}</b><span className={`mt-1 block text-[10px] ${action===a.value?"text-blue-100":"text-slate-400"}`}>{a.help}</span></button>)}
          </div>
          <label className="mt-4 block text-[10px] font-black uppercase tracking-wide text-slate-500">Asset tag / barcode</label>
          <input ref={inputRef} autoFocus value={identifier} onChange={e=>setIdentifier(e.target.value)} onKeyDown={e=>{if(e.key==="Enter"){e.preventDefault();scan()}}} placeholder="Scan tag and press Enter…" className="mt-2 w-full rounded-xl border border-slate-300 px-4 py-3 text-base font-black outline-none ring-blue-100 focus:border-blue-500 focus:ring-4"/>
          <input value={notes} onChange={e=>setNotes(e.target.value)} placeholder={action==="damaged"||action==="missing"?"Condition / exception note…":"Optional scan note…"} className="mt-2 w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm"/>
          <button disabled={busy||!identifier.trim()} onClick={scan} className="mt-3 w-full rounded-xl bg-slate-950 px-4 py-3 text-sm font-black text-white disabled:opacity-40">{busy?"Saving scan…":`${ACTIONS.find(a=>a.value===action)?.label} scanned asset`}</button>
