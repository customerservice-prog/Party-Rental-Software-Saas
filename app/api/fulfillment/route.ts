...
export async function POST(req:NextRequest){
  ...
  if(action==="setCounts"){
    ...
    const notes=typeof body.notes==="string"?body.notes.trim().slice(0,1000):fulfillment.notes;
    ...
    await prisma.$executeRawUnsafe(
      ...
      proofName||null,proofSignature,proofPhotoUrl,notes||null,fulfillment.id,organization.id
    );
    await prisma.$executeRawUnsafe(
      `INSERT INTO "RentalFulfillmentEvent" ("id","organizationId","orderId","fulfillmentId","type","notes","performedBy") VALUES ($1,$2,$3,$4,'proof_updated',$5,$6)`,
      randomUUID(),organization.id,orderId,fulfillment.id,
      `name=${proofName?"yes":"no"}; signature=${proofSignature?"yes":"no"}; photo=${proofPhotoUrl?"yes":"no"}; notes=${notes?"yes":"no"}`,
      user.id
    );
  }
