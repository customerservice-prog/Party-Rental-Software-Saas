...
export async function PATCH(
  ...
  if ("defaultStopPay" in body) {
    const raw = body.defaultStopPay;
    const num = Number(raw);
    if (raw === null || raw === "" || isNaN(num) || num < 0) {
      return NextResponse.json({ error: "Enter a valid dollar amount for default stop pay." }, { status: 400 });
    }
    data.defaultStopPay = num;
  }
