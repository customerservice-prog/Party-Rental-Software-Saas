// Production preparation is additive. A schema push is allowed only for a
// genuinely empty database. Existing databases are never reset or synchronized
// destructively; future core-schema changes require a reviewed migration.
...
async function main(){
 ...
 }else if(!tables.some(t=>t.tablename==='Organization')||!tables.some(t=>t.tablename==='User'))throw Error('Existing nonempty database is not an initialized CRM schema. No schema reset was attempted.');
 script('ensure-fulfillment-schema.js');
 // Ensure the shipped client can read all declared core fields. Missing fields
 // stop the release and request an explicit migration rather than dropping data.
