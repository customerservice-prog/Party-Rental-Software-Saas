...
async function tick(db,{send,now=()=>Date.now()}={}){
 ...
 try{
  ...
  for(const t of tenants){if(now()-started>150000)break;summary.tenantsChecked++;try{const result=await runBookingBatch(db,t.organizationId,{source:'scheduled',...(send?{send}:{})});summary.attempted+=result.attempted;summary.accepted+=result.confirmationsSent+result.remindersSent+result.balanceRemindersSent;summary.failed+=result.failed;summary.unknown+=result.unknown;}catch{summary.errors++;}}
