...
async function claim(db,orgId,orderId,kind,channel,source,leaseToken){
 return db.$transaction(async tx=>{
  ...
  if(channel==='email'&&(!policy.emailEnabled||!org.resendApiKey||!org.senderEmail)||channel==='sms'&&(!policy.smsEnabled||!org.twilioAccountSid||!org.twilioAuthToken||!phone(org.twilioFromNumber)))return null;
...
async function runBookingBatch(db,orgId,{source='activity',send=providerSend}={}){
 const result={ran:false,confirmationsSent:0,remindersSent:0,balanceRemindersSent:0,attempted:0,failed:0,unknown:0};
 ...
 try{
  ...
  const orders=await db.order.findMany({where:{organizationId:orgId,status:{in:['confirmed','active']},eventDate:{gte:today},...(source==='scheduled'?{createdAt:{gte:new Date(policy.enabledAt)}}:{}),OR:filters},include:{customer:true},orderBy:[{eventDate:'asc'},{createdAt:'asc'},{id:'asc'}],take:100});
  result.ran=true;const counted=new Set();
  for(const order of orders)for(const kind of ['booking_confirmation','balance_due','event_reminder']){
   ...
   for(const channel of ['email','sms']){
    ...
    if(outcome.state==='accepted'){const key=order.id+kind;if(!counted.has(key)){counted.add(key);result[TYPES[kind].count]++;}}
...
module.exports={runBookingBatch,providerSend,windowOpen,eligible,phone,message,claim,finishDelivery};
